...
app.post('/api/auth/student', async (req,res) => {
  try {
    ...
    const user = q.rows[0];
    if (!user || !(await bcrypt.compare(String(password), user.password_hash))) return res.status(401).json({ error:'NISN atau password salah.' });
    res.json({ token: tokenFor(user), user: publicUser(user) });
  } catch (err) {
    console.error('Student login failed:', err);
    res.status(500).json({ error:'Server gagal memproses login siswa.' });
  }
});
app.post('/api/auth/admin', async (req,res) => {
  try {
    ...
    const user = q.rows[0];
    if (!user || !(await bcrypt.compare(String(password || ''), user.password_hash))) return res.status(401).json({ error:'Username atau password admin salah.' });
    res.json({ token: tokenFor(user), user: publicUser(user) });
  } catch (err) {
    console.error('Admin login failed:', err);
    res.status(500).json({ error:'Server gagal memproses login admin.' });
  }
});
...
async function start() {
  // Bind the port first so Abasthan can detect the service even when
  // database initialization takes time or temporarily fails.
  app.listen(PORT, '0.0.0.0', () => console.log(`WebClass API listening on :${PORT}`));
  try {
    await initDb();
